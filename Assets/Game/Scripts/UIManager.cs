...
namespace Game.Scripts
{
    public class UIManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            int specialImagesID = (_lvlNum - 1) / 5;
            if (specialImagesID < _specialImages.Count)
            {
                _lvlNumImages[^1].GetComponent<Image>().sprite = _specialImages[specialImagesID];
                _lvlNumImages[^1].GetComponentInChildren<TMP_Text>().gameObject.SetActive(false);
            }
            else
            {
                _lvlNumImages[^1].GetComponentInChildren<TMP_Text>().text = "70";
            }
            LevelNumberHandler();
